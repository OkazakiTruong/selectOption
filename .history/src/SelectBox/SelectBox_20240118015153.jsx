...
export const SelectBox = ({
  ...
  const handleEnter = (e) => {
    ...
    if (e.key === "Backspace" || e.key === "Delete") {
      if (optBox?.length && optBox !== null) {
        handleDeleteOptItems(e, optBox[optBox?.length - 1].id);
      }
    }
  };
  useEffect(() => {
  ...
  return (
    <div ref={wrapperRef}>
      ...
      <div className="select-option" ref={selectOptionRef}>
        {console.log(optBox.length)}
        {optBox === null || !optBox.length ? (
          <div className="option-item">
            <FontAwesomeIcon icon={faServer} className="server-icon" /> Không có
            dữ liệu
          </div>
        ) : (
          optBox?.map((optItem) => {
